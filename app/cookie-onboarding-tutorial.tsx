...
const steps=[
 ...
 {title:"Find Your People",text:"Use the Friends search bar to search by username and connect with people on Cookie.",target:"friends"},
...
function findTarget(t?:string){
 ...
 if(t==="filters"){
   ...
   if(buttons.length>=3){let p:HTMLElement|null=buttons[0].parentElement;while(p&&p!==document.body){const text=(p.textContent||"").toLowerCase();if([...labels].every(x=>text.includes(x)))return p;p=p.parentElement}return buttons[0].parentElement||buttons[0]}
   return null;
 }
