...
function extractStoryPath(src:string){
  if(!src)return "";
  const marker="/story-media/";
  const i=src.indexOf(marker);
  if(i<0)return "";
  return decodeURIComponent(src.slice(i+marker.length).split("?")[0].replace(/["')]+$/g,""));
}
...
export default function CookieStoryViewers(){
 ...
 useEffect(()=>{let stopped=false;let timer:number|undefined;
   ...
   const sync=async()=>{if(stopped)return;const root=document.querySelector(".story-viewer");if(!root){setOwnStory(null);setOpen(false);lastKey.current="";return}
     ...
     const key=`${story.id}:${user.id}`;if(key===lastKey.current){if(story.author_id===user.id){setOwnStory(story as Story);await load(story.id)}return}lastKey.current=key;
