...
export default function ChatThemeControlV2(){
  ...
  useEffect(()=>{const tick=()=>{const k=key();if(k&&k!==current.current){current.current=k;const v=load(k);setSaved(v);paint(v)}if(k){const h=document.querySelector('.chat-head');if(h&&!h.querySelector('[data-theme-btn]')){const b=document.createElement('button');b.textContent='Theme';b.setAttribute('data-theme-btn','1');b.className='zale-theme-btn';h.appendChild(b)}}};tick();const id=setInterval(tick,400);const click=(e:Event)=>{if((e.target as HTMLElement).closest('[data-theme-btn]'))setOpen(true)};document.addEventListener('click',click);return()=>{clearInterval(id);document.removeEventListener('click',click)}},[]);
  useEffect(()=>{const st=document.createElement('style');st.textContent='.zale-theme-btn{border:1px solid #e2daf4;background:#fff;color:#5d35bd;border-radius:12px;padding:8px 10px;font-weight:800}.zale-theme-modal{position:fixed;z-index:9999;inset:0;background:#1a102b66;display:grid;place-items:center;padding:18px}.zale-theme-card{width:min(440px,100%);background:#fff;border-radius:24px;padding:20px;box-shadow:0 28px 80px #1f10374d}.zale-theme-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.zale-theme-choice,.zale-theme-upload{height:82px;border:1px solid #dfd6f5;border-radius:18px;display:grid;place-items:center;font-weight:900;background-size:cover!important}.zale-theme-upload{border-style:dashed;cursor:pointer}.zale-theme-upload input{display:none}.zale-theme-close{width:100%;margin-top:14px;border:0;background:#281641;color:#fff;border-radius:14px;padding:12px;font-weight:900}@media(max-width:760px){.zale-theme-btn{font-size:0;width:38px;height:38px;padding:0}.zale-theme-btn:after{content:"◐";font-size:18px}}';document.head.appendChild(st);return()=>st.remove()},[]);
