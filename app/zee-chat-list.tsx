...
export default function ZeeChatList(){
  useEffect(()=>{
    ...
    const mount=()=>{
      ...
      if(buttons.length>1) list.insertBefore(row,buttons[1]); else list.appendChild(row);
    };
    const scheduleMount=()=>{ if(scheduled) return; scheduled=true; window.requestAnimationFrame(mount); };
    mount();
